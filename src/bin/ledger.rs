...
use hodeauxledger::disk;
use hodeauxledger::key::{sign_as_author, sign_as_quorum, sign_as_usher};
use hodeauxledger::rhex::{Intent, Rhex, gen_nonce};
use hodeauxledger::{b64::from_base64_to_32, rhex};
...
fn craft_intent(args: &Cli) -> anyhow::Result<(), anyhow::Error> {
    ...
    let nonce = args.nonce.clone().unwrap_or_else(gen_nonce);
...
fn verify_current_hash(args: &Cli) -> anyhow::Result<(), anyhow::Error> {
    let rhex_path = args
        ...
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("rhex must be specified"))?;
    let mut rhex = disk::load_rhex(&Path::new(rhex_path).to_path_buf())?;
    rhex.verify_hash()?;
...
fn create_genesis(args: &Cli) -> anyhow::Result<(), anyhow::Error> {
    ...
    // Sign as author
    let intent_hash = blake3::hash(&rhex.intent.canonical_bytes()?);
    rhex.signatures.push(rhex::Signature {
        sig_type: 0,
        public_key: pk_bytes,
        sig: sign_as_author(intent_hash.as_bytes(), &sk).into(),
    });
    let mut buf = rhex.signatures[0].sig.to_vec();
    buf.extend_from_slice(rhex.context.at.to_be_bytes().as_slice());

    // Sign as usher
    let usher_hash = blake3::hash(&buf);
    rhex.signatures.push(rhex::Signature {
        sig_type: 1,
        public_key: pk_bytes,
        sig: sign_as_usher(usher_hash.as_bytes(), &sk).into(),
    });

    // Sign quorum
    let mut buf = rhex.signatures[0].sig.to_vec();
    buf.extend_from_slice(&rhex.signatures[1].sig.to_vec());
    let quorum_hash = blake3::hash(&buf);
    rhex.signatures.push(rhex::Signature {
        sig_type: 2,
        public_key: pk_bytes,
        sig: sign_as_quorum(quorum_hash.as_bytes(), &sk).into(),
    });
