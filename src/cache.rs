...
pub fn store_key(
    ...
    let mut stmt =
        conn.prepare("INSERT INTO keys (key, roles, scope, expiry) VALUES (?1, ?2, ?3, ?4)")?;
...
pub fn retrieve_key(
    ...
    // Coerce &[u8;32] -> &[u8] for BLOB binding
...
pub fn revoke_key(conn: &Connection, key: &[u8; 32], scope: &str) -> anyhow::Result<()> {
    let mut stmt = conn.prepare("DELETE FROM keys WHERE key = ?1 AND scope = ?2")?;
    stmt.execute(params![&key[..], scope])?;
    Ok(())
}

pub fn flush_scope_keys(conn: &Connection, scope: &str) -> anyhow::Result<()> {
    let mut stmt = conn.prepare("DELETE FROM keys WHERE scope = ?1")?;
    stmt.execute(params![scope])?;
    Ok(())
}

pub fn flush_all_keys(conn: &Connection) -> anyhow::Result<()> {
    let mut stmt = conn.prepare("DELETE FROM keys")?;
    stmt.execute([])?;
    Ok(())
}
